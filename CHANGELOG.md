...
- **Rewrite stops completing essays and Koreanizing terms** (#874). The rewrite prompt now keeps Latin-script technical terms, refuses to invent a "why" or a lesson the source never had, and stops filling a heading-only draft into essay prose. Adds document types `resume`, `personal-statement`, and `project-writeup`.
- **Inspect gains an advisory layer** (#874). `patina inspect` accepts `--document-type` and `--rewrite`, and its JSON gains `documentType`, `structureFingerprint`, `discourseShape`, and `advisories`. The additions are advisory-only: they stay outside `analyzeText().hot` and the deterministic score, and `schemaVersion` is unchanged because the payload is additive.
- **Stricter meaning safety.** An empty-anchor MPS no longer certifies a source that carries numeric claims, closing a path where a schema-valid score could mark a polarity inversion as `verified: true` (#871). The CLI rewrite path now enforces `numeric_claim_changed` with a new `numeric-claim-changed` verification reason, so a sign flip, a word-number swap, an added numeric claim, or a collapsed duplicate fails on the CLI the way it already failed on the hosted path (#870). `dropped-numbers` keeps precedence; `unsupported_numeric_syntax` stays web-only, so identity comparators such as `p < 0.05` still pass on the CLI.
- **Pro pack metering charges only delivered downloads** (#867). The daily cap was incremented on every authenticated GET, so a listing, a 404, a sha mismatch, and an upstream outage each cost a unit and one `patina pack install` cost two. It now charges once, after the integrity check passes. Manifest path validation also rejects encoded and non-canonical paths as defense in depth.
- **New advisory notes, warning only.** An overcorrection guard flags a rewrite that removed one slop class and added another — every typographic dash stripped from a dash-leaning source, or slang the source never used (#882). A portability probe flags a draft whose sentences would stay true if the product, company, or person were swapped (#881); it is detect-only by design, since the rewrite-side form of that constraint was tested and not supported in `docs/research/2026-rewrite-efficacy-study4.md`. Both are advisory: exit codes, scores, and rewrite text are unchanged. Disable with `overcorrection-guard: false`.
- **2026 cadence detection for short-form English** (#879). The short-form signal now recognises the combination — short sentence stacks, parallel set groups, informationless aside dashes, all-caps closers, and phrasings like "that matters" — instead of treating a lone em dash as the whole tell. Numeric ranges and quoted literary dashes are explicitly cold. Audit/score only; rewrite behavior is unchanged. Promotion is gated on a 50-document fixture, which now ships and measures precision 1.00 / recall 0.76 against it (#893).
- **Pattern 31 covers thematic commentary** (#875). The conclusion-signal pattern now also names closers that only restate the theme of claims the body already made, in all four languages. Pack counts and catalogs are unchanged; no new pattern number was minted.
- **Docs and contracts.** `FLAG-PARITY.md` is refreshed against the shipped 8.7.1 CLI and now lists `patina inspect`, `patina aside`, and the `agy-cli` backend; `CLI.md` gains a command index and no longer tells users to inspect a draft with `--diff` (#868). The v6.2 changelog claim that the meaning proxy rides the persona report JSON is corrected in place — that wiring never shipped, and meaning safety stays outside Persona metadata (#873).
- **Internal.** `src/inspection.js` imports `detectLanguage` from `src/prose-core.js` instead of reaching into `scripts/`, and the now-unreachable architecture exception is removed (#869). TypeScript 7 adoption with JSDoc describing real shapes (#865), an explicit strict tsconfig default (#862), a hold on TypeScript major bumps until TS 7 readiness (#862), `docker/metadata-action` v6 (#860), and the 2026-09-09 plan archive (#866).
...
- **Deterministic meaning-floor proxy (advisory, Phase A)**: `src/features/meaning-proxy.js` is a Lane A, LLM-free (module-boundary tested) `evaluateMeaningProxy({original, rewrite, lang})` that flags likely meaning drift without a model — dropped numbers, rare-content-token recall (active only with ≥3 rare tokens; recall <0.5 warn / <0.3 fail), negation-polarity delta (word/token-boundary, not raw substring, so "cannot"/"notable"/안전/안내 are not miscounted), and length-ratio extremes ([0.4, 2.5]). It rides the persona report JSON as `meaning_proxy` and the gate's advisory list. **Correction (2026-09-16, #873):** that persona-report wiring never shipped and was dropped from the later design — `buildPersonaReport` emits only `id`, `thresholds_source`, `match`, `over_edit_churn` and `gate_result`, and `evaluatePersonaGate` deliberately ignores meaning safety, which `tests/unit/persona-gates.test.js` and `tests/e2e/cli-persona.test.js` fence. Meaning safety stays on the global rewrite/`--verify` path, outside Persona metadata. **Phase A is advisory-only** — no CLI warning and no exit-code change (dropped numbers stay separately enforced); promotion to enforcing awaits the formal 2-round calibration. No detection-path (`analyzeText`) change — benchmark accuracy is unaffected.
