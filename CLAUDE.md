...
- `patina-max/SKILL.md` — 설치 가능한 MAX mode 엔트리포인트 (`claude -p` / `gemini -p` + `codex exec`)
- `.patina.default.yaml` — 기본 설정 (language, patterns, profile, max-models, dispatch comments)
- `core/voice.md` — 문체/개성 가이드라인
- `core/scoring.md` — 스코어링 알고리즘 레퍼런스 (심각도 루브릭, 카테고리 가중치, 공식)
- `patterns/ko-*.md` — 한국어 AI 패턴 탐지 팩 (28개 패턴, 6개 팩)
- `patterns/en-*.md` — 영어 AI 패턴 탐지 팩 (28개 패턴, 6개 팩)
- `patterns/zh-*.md` — 중국어 AI 패턴 탐지 팩 (28개 패턴, 6개 팩)
- `patterns/ja-*.md` — 일본어 AI 패턴 탐지 팩 (28개 패턴, 6개 팩)
...
- `SKILL.md`, `SKILL-MAX.md`, `patina-max/SKILL.md`, `.patina.default.yaml`, `README.md`의 version 필드는 동기화 필수
