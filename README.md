...
The score is pattern-based and deterministic — it reuses the same 32 (Korean), 31 (English), 31 (Chinese), or 32 (Japanese) detection patterns from audit mode. Profile overrides affect scoring (e.g., blog profile suppresses bold pattern #14).
...
All four languages share the same 6-category structure (126 patterns total: 32 KO + 31 EN + 31 ZH + 32 JA). The categories and most patterns are universal — only a few slots have language-specific variants. Patterns #30 (rhetorical question openers) and #31 (conclusion signal words) exist in all four languages. Pattern #32 (comparative adverb overuse — KO `보다`, JA `より`) is KO/JA-specific; English and Chinese don't have an equivalent particle pattern.
...
| Version | Changes |
|---------|---------|
| **3.8.0** | Korean lexicon re-curation. Track A's paired ko/AI corpus (100 Claude-generated Korean paragraphs from NamuWiki topic seeds) revealed Korean lexicon contributed only +1pp on AI catch (vs +10pp on English) under v3.7.0. Differential-frequency mining against NamuWiki human prose (AI doc-freq ≥4×, ratio ≥4.0) surfaced 12 new high-signal entries: `평가된다`, `꼽힌다`, `가리킨다`, `사례로`, `다수의`, `알려져`, `일컬어진다`, `평가받다` (strict, 8) and `가운데 하나로`, `자리 잡았다`, `알려져 있다`, `~의 사례로` (phrases, 4). Korean lexicon now 49 strict + 54 phrases = 103 entries (formal count from header is 102; one phrase is `~의 사례로` redundant with strict `사례로`). Result: ko/AI catch 83% → **91%** (+8pp). NamuWiki human FP held at **13%** (zero regression — clean Pareto win). English lexicon and English numbers unchanged (en lex untouched). Algorithm reference: `core/stylometry.md` §16; mining script: `.omc/research/v3_8_ko_lexicon_mine.py`; reproduction: `.omc/research/v3_8_remeasure.py`. |
| **3.7.0** | AI-lexicon overlap signal (new step 4.7). A flat dictionary (`lexicon/ai-en.md` 108 entries, `lexicon/ai-ko.md` 90 entries) flags AI-favored phrases the 28-pattern catalog does not enumerate. Densities are computed per 1000 tokens; the 4.6 hot rule extends to a 3-signal OR (burstiness OR MATTR OR lexicon_density > 2.0). Calibrated against 400 paragraphs (HC3 ChatGPT/human + Wikipedia + NamuWiki): AI catch lifts from 66% to **76%** with HC3 human FP 12%→19%, Wikipedia FP 23%→**25%** boundary, NamuWiki FP 11%→13% (within +5pp guardrail). All acceptance gates met (AI ≥75%, max FP ≤25%, NamuWiki regression ≤+5pp) — first Pareto improvement over the v3.5.1 wall. Drop list (post-eval): `intersection`, `principles`, `mindset`, `iterative`, `responsible`, `methodologies`, `redefine`, `accessible`, `equitable`, `one of the most`, `in conjunction with`, `the power of` — fired more on academic prose than on AI text. Skipped v3.6 (n-gram dropped, §15 negative finding). Algorithm reference: `core/stylometry.md` §16; calibration script: `.omc/research/v3_7_lexicon_eval.py`. |
| **3.5.1** | Stylometric calibration patch. Raised `stylometry.burstiness.bands.low` from 0.25 to 0.30 after external validation against 300 paragraphs (HC3 ChatGPT 100 + HC3 human 100 + Wikipedia 100). v3.5.0 only caught 57% of real AI text; v3.5.1 catches 66% with HC3 human FP 12% and Wikipedia FP 23%. Sweep showed no threshold combo satisfies both AI ≥70% and max FP ≤20% — Wikipedia's encyclopedic register naturally has uniform sentence length. MATTR threshold unchanged (0.55) because sweep proved it has no discriminative power on prose. Honest framing: v3.5.x is an advisory marker for the LLM, not a sole-decision gate. n-gram repetition (originally v2 roadmap) elevated in priority for v3.6+. Calibration evidence in `core/stylometry.md` §13. |
