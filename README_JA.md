...
韓国語（32）、英語（31）、中国語（29）、日本語（29）の合計121パターンを検出します。[全パターン一覧](#パターン)は下記をご覧ください。
...
スコアはパターンベースで決定的です。audit モードと同じ32（韓国語）、31（英語）、29（中国語）、29（日本語）の検出パターンを再利用します。プロファイルのオーバーライドはスコアリングに影響します（例：blog プロファイルでは太字パターン #14 が抑制されます）。
...
4言語すべてが同じ6カテゴリ構造を共有します（合計121パターン：韓国語 32 + 英語 31 + 中国語 29 + 日本語 29）。カテゴリと大半のパターンは言語を超えて共通です。一部のスロットのみ言語固有の実装があります。パターン #30（修辞疑問の段落冒頭）と #31（結論シグナルワード）は現在 KO/EN のみで、パターン #32（「보다」比較副詞の濫用）は KO 専用です（英語に同等の助詞がないため）。ZH/JA の #30/#31 展開は今後の作業です。
