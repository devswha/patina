...
**変換前**（AI臭い文章）：
> AIコーディングツールは、大規模言語モデルの**画期的な可能性**を示す**革新的なマイルストーン**であり、ソフトウェア開発の進化における**重要な転換点**を意味します。これによりプロセスの効率化のみならず、コラボレーションの促進や組織の連携強化が実現されます。

**変換後**（人間らしい文章）：
> AIコーディングツールを使うと、雑用は早く片付く。設定ファイルやテストの雛形とか、そういうやつ。問題は、出力されたコードが正しく見えてしまうこと。コンパイルも通るしlintも通るから、そのままマージしてしまう。後から気づくと、意図とまったく違う動作をしていた。
...
各モデルが独立して人間化処理を行い、AI類似度スコアで採点し、最もスコアが低い（最も人間らしい）結果が採用されます。
...
忠実度は4つの基準で判定されます：主張の保持、捏造の有無、トーンの一致、長さの比率。総合スコアは両方の軸を加重平均します。プロファイルごとに設定可能です（例：academic は忠実度 0.60、AI 0.40、blog は AI 0.70、忠実度 0.30）。
...
入力テキスト
  |
  v
[Phase 1] 構造スキャン -- 段落レベルの問題を修正（繰り返し、受動態）
  |
  v
[Phase 2] 文レベルの書き換え -- 語彙レベルの問題を修正（AI語彙、フィラー、ヘッジング）
  |
  v
[Phase 3] セルフ監査 -- 「まだAIっぽくないか？」-- 残りの問題を修正
  |
  v
自然な文章
```

言語固有のパターンパック（`ko-*.md`、`en-*.md`、`zh-*.md`、`ja-*.md`）を読み込み、この3フェーズパイプラインで適用します。プロファイルとボイスガイドラインがトーンを調整します。
...
├── core/scoring.md           # スコアリングアルゴリズムリファレンス
├── patterns/
