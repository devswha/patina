...
## 브라우저 playground — 설치 없음

**[patina.vibetip.help](https://patina.vibetip.help/)** 에서 KO / EN / ZH / JA 문장을 붙여넣으면 브라우저에서 바로 실제 리라이트를 받아볼 수 있습니다. 결과에는 MPS/충실도 하한 검증과 결정론적 AI 시그널(before → after)이 함께 표시됩니다.

> **리라이트와 채점은 서버에서 실행됩니다.** 무료 모드는 서비스 자체 모델 키로 동작하며 요청량이 제한됩니다. **API 모드**의 개인 키는 브라우저 저장소에만 보관되고, 요청마다 patina 서버를 경유해 선택한 프로바이더로 전달될 뿐 서버에 저장되거나 로깅되지 않습니다(서버 메트릭은 텍스트·프롬프트·출력·키·IP를 기록하지 않습니다).
...
**코딩 에이전트에게 설치를 맡기세요** — Claude Code, Codex CLI, Cursor, Gemini CLI 등에 아래를 붙여넣으세요:

```text
Install patina by following https://raw.githubusercontent.com/devswha/patina/main/INSTALLATION.md
```

에이전트가 [`INSTALLATION.md`](INSTALLATION.md)(AI 에이전트용 가이드)를 가져와 사용 환경에 맞는 설치 경로를 실행하고 검증합니다. 직접 하려면:
