...
**적용 후** (자연스러운 글):
> AI 코딩 도구는 단순 반복 작업을 빠르게 처리해줍니다. 설정 파일이나 테스트 뼈대 같은 것들이요. 문제는 코드가 겉보기에 멀쩡하다는 겁니다. 컴파일도 되고 린트도 통과하니까 머지하는데, 나중에 보면 의도와 전혀 다른 동작을 하고 있습니다.

한국어(28개), 영어(28개), 중국어(28개), 일본어(28개) 총 112개 패턴을 탐지합니다. [전체 패턴 목록](#patterns)은 아래에서 확인하세요.
...
각 모델이 독립적으로 텍스트를 사람답게 고치고, AI 유사도 점수를 매긴 뒤, 가장 낮은 점수(가장 사람다운 결과)가 선택됩니다.
...
충실도는 네 가지 기준을 확인합니다: 주장 보존, 허위 내용 없음, 톤 일치, 길이 비율. 종합 점수는 두 차원을 가중 합산하며, 프로필별로 설정할 수 있습니다(예: 학술: 충실도 0.60, AI 0.40 / 블로그: AI 0.70, 충실도 0.30).

점수는 패턴 기반의 결정적 방식으로 계산됩니다. 감사(audit) 모드와 동일한 28개(한국어), 28개(영어), 28개(중국어), 28개(일본어) 탐지 패턴을 재사용합니다. 프로필 오버라이드는 점수에 영향을 줍니다(예: 블로그 프로필은 볼드 패턴 #14를 억제).
...
입력 텍스트
  |
  v
[Phase 1] 구조 스캔 -- 문단 수준 문제 교정 (반복, 수동태)
  |
  v
[Phase 2] 문장 재작성 -- 단어 수준 문제 교정 (AI 어휘, 군더더기, 헤징)
  |
  v
[Phase 3] 자기 감사 -- "아직 AI처럼 들리는가?" -- 남은 문제 교정
  |
  v
자연스러운 텍스트
```

스킬은 언어별 패턴 팩(`ko-*.md`, `en-*.md`, `zh-*.md`, `ja-*.md`)을 불러와 이 3단계 파이프라인에 적용합니다. 프로필과 보이스 가이드라인이 톤을 결정합니다.
...
<details>
<summary><b>소통 패턴</b> -- 챗봇 흔적 3개 패턴</summary>
...
<details>
<summary><b>소통 패턴</b> -- 3개 패턴</summary>
...
├── core/scoring.md           # 스코어링 알고리즘 참고
├── patterns/
│   ├── ko-*.md               # 한국어 패턴 (6개 팩, 28개 패턴)
│   ├── en-*.md               # 영어 패턴 (6개 팩, 28개 패턴)
│   ├── zh-*.md               # 중국어 패턴 (6개 팩, 28개 패턴)
│   └── ja-*.md               # 일본어 패턴 (6개 팩, 28개 패턴)
