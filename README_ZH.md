...
共检测 125 个模式，覆盖韩文（32 个）、英文（31 个）、中文（31 个）和日文（31 个）。完整模式列表见[下文](#模式)。
...
评分基于模式匹配，结果确定——复用审计模式中的 32 个（韩文）、31 个（英文）、31 个（中文）或 31 个（日文）检测模式。配置文件覆盖会影响评分（例如：博客配置文件会屏蔽粗体模式 #14）。
...
四种语言共享相同的 6 大类结构（共 125 个：韩文 32 + 英文 31 + 中文 31 + 日文 31）。各类别及大多数模式是通用的——只有少数槽位有语言特定的实现。模式 #30（修辞疑问句段首）和 #31（结论信号词）现已覆盖全部 4 种语言。模式 #32（"보다" 比较副词滥用）为韩文专属（英文没有对应助词）。
