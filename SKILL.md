...
description: |
  ...
  사람이 쓴 것처럼 만듭니다. 플러그인 기반 구조로 패턴 팩과 프로필을 조합합니다.
...
프로필이 없으면 `profiles/default.md`를 사용한다.
...
로드된 패턴, 프로필, 목소리 지침을 조합하여 텍스트를 처리한다.

### 작업 지침

텍스트를 받으면:

1. **AI 패턴 식별** - 로드된 패턴 팩의 모든 패턴을 스캔
...
7. **최종 AI 검수** - "아래 글에서 AI가 쓴 것처럼 보이는 부분은?" 질문 후 짧게 답하고, "이제 AI가 쓴 것처럼 보이지 않게 고쳐라" 지시 후 수정
