...
이번 검증에서 한국어 AI 텍스트는 측정하지 못했다. 자유 배포 가능한 한국어 AI 단락 코퍼스(HC3·RAID 동급) 가 없기 때문. 후속 작업: NamuWiki 단락과 같은 주제로 GPT-4/Claude 를 직접 호출해 paired 한국어 AI 100 단락을 생성하고 측정. 생성 프롬프트와 모델·날짜를 메타데이터로 기록해 재현성 확보.
