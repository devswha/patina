...
> **Mode support:** `codex-cli`, `claude-cli`, `gemini-cli`, and `agy-cli` can be used as rewrite backends without `PATINA_API_KEY` when their local CLIs are already authenticated.
