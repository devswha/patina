...
| issue | status | next action |
|---|---|---|
| #303 KO stylometry | partially implemented; 10-row hash-only web candidate manifest and pilot template ready | Score the hash-only web human-control candidates locally, then fill the remaining 25-row KO pilot before comparing KatFish-style spacing/comma signals against the local manifest. |
...
| #157 Korean register FP | partially unblocked; hash-only web human-control candidates ready | Review/score `artifacts/rebaseline-2025/human-controls.public.jsonl`, then add enough academic/종결-다 controls to reach the 25-row KO pilot before loosening or tightening KO thresholds. |
