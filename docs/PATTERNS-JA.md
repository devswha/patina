...
例文は文体の説明用で、入力の事実を検証した資料ではありません。`success` と `failure` の各ファイルは独立した事例です。`failure` は誤検出を避けるための無修正例であり、同じ番号の `success` を書き換えた結果ではありません。

例の点検はモデルによる編集上の検討で、人による評価や実測の意味保持スコアではありません。文脈が足りない例は、その箇所に修正・発火判定を保留する理由を記しています。
...
| 34 | rewrite | 無生物行為主（擬似的行為性） | [ja-language.md](../patterns/ja-language.md) |
...
> プロジェクトの日程は、もっと具体的なマイルストーンを設定する必要がありそうです。予算配分についても、今より効率よく運用する方法を検討する必要があると考えます。可能であれば、来週中にもう少し深く議論できるとありがたいです。
...
**修正後：**
> 対称性は信頼を支える。一貫性は、ブランドと顧客の関係を支えるものだ。デザインの細部には、その企業の価値観が表れる。
...
**注意語彙:** データが物語っている, 数字が証明している, 決定が自然と生まれた, 文化が変わっていった, 議論が~へと動いた, 市場が報いる, 市場が選択する, このテクノロジーが~を可能にする, 戦略が要求する, 状況が語るように, 結果がすべてを物語る

**発火条件:** 同一文書内で、無生物主語が人間の行為（決定・判断・報酬・要求・議論の主導）を行う文が 2 回以上現れ、文脈から行為者を特定できるのに最後まで名前が出てこないとき。単独 1 回は audit ヒントのみ。

**除外条件:**
- 学術・報告書の引用慣行（「本研究は~を示す」「報告書は~を指摘する」）— 著者への慣用的な換喩
- 本当に行為者のない過程（「気温が上がった」「締め切りが過ぎた」「市場が暴落した」のような集合的事象）
- 文学的装置として一貫して維持される擬人法、定着した慣用句
- ソフトウェア/システムが実際に行う動作（「リンターが検出する」「パイプラインがリトライする」）

**受動態検出との区別:** 受動態は文法で行為者を隠す（「決定が下された」）。擬似的行為性は客体を主語に昇格させて隠す（「決定が自然と生まれた」）。残る問いは同じ——誰がやったのか——だが、このパターンは能動態の文で発火するため、受動態検出では捕まらない。もともと日本語は無生物主語構文を好まない言語なので、この構文の蓄積は翻訳調のシグナルとしても機能する。

**Semantic Risk:** HIGH
**Preservation Note:** 原文が裏付けない行為者を捏造しない。文脈から行為者が復元できるなら（チーム、筆者、「あなた」）それを使い、復元できないなら既知の事実を中心に文を組み直す。行為者の捏造は文体修正ではなく意味違反である。

**問題:** AI は抽象物を行為者に昇格させる——決定が生まれ、文化が変わり、データが語り、市場が報いる。誰が何をしたかにコミットせずに分析的な響きを出せるからだ。人間の文章は行為者を名指すか、少なくともどこかに立つ（「私たち」「あなた」「チーム」）。手がかりは一文ではなく蓄積にある：段落全体で出来事は起きているのに、人がひとりも登場しない。

**修正前:**
> フィードバックループが品質より速度に報いた結果、四半期のうちに文化が変わっていった。意思決定は会議ではなく Slack スレッドから自然と生まれ、ロードマップは指標が要求する方向へ曲がっていった。

**修正後:**
> フィードバックループの中で品質より速度が評価された結果、四半期のうちに文化が変わっていった。会議ではなく Slack スレッドで自然と意思決定が進み、ロードマップは指標に沿う方向へ変わっていった。

**例の注記：** この抜粋だけでは行為者を特定できず、Pattern 34 の発火条件を満たすとは断定できない。受動表現で出来事を整理した例であり、経営陣や担当職種を推測する例ではない。
...
**例の注記：** 具体的な計画のない原文から予定や地名を補わない。期待は残るため、楽観表現をすべて除去した例ではない。
...
- Context guard: issue #352 — 翻訳調が em dash、コロン、スラッシュ、読点接続と一緒に出る場合、記号だけを置き換えず、`無 TUI 設定` → `TUIなしで完全自律の設定にしたい場合は...` のように節構造を自然な日本語に組み直す。
...
> 広告ゼロでなぜここまで広がったのか？
> 
> （4段落に分かれる例。ただし空白を除く各行は15・11・18・18字で平均15.5字となり、この抜粋だけでは「平均文長が15字未満」の発火条件を満たさない。）
...
**変更前 / 変更後例（既定のリライトでは変更しない）：**

> 変更前：研究によると、創業者の 73% はこの習慣を無視して損をしている。
>
> 変更後：研究によると、創業者の 73% はこの習慣を無視して損をしている。

例の注記：スコア・監査の対象となる表現でも、このパックは既定のリライトでは変更しない。例文の主張が正しいと検証したわけではなく、出典や別の事実を補っていない。
...
>
> 変更後：スタンフォード出身、Y Combinator 支援の連続起業家が語る、成長を 10 倍にするワークフロー。
