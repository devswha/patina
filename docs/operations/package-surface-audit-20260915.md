...
- **Audited tree:** `a1116bfc74d8d75aa2d4ebbc7e4a9c1d3a0f4178` (final
  converged-chain refresh, 2026-09-16: merge of the audit branch `f6d5a8d…`
  with `origin/dev` `3622d3bb8667287cc089ce37b12d7bc8c385e72b`, after
  PR #847, #849, #850, and #851 — the remainder of the 2026-09-15
  maintenance chain — merged; PR-06 was retired by the owner without ever
  being defined or started, so the chain is complete; main ancestry verified) in worktree
  `../patina-maint-07`, branch `bot/maint-07-package-surface-audit`. The
  initial audit ran at `67e0cd85f44500c52f0f708ed83bbfad5db33e63` and was
  first refreshed at `3c078b11cad206265c19855117a3021a15952d65` (PR #846);
  every measurement below was re-run at this final tree because the merged
  chain changed shipped `src/backends/*` and `docs/` files (#847, #849,
  #851).
...
|---|---:|---:|---:|---|
| `patina-cli@8.7.0` (root) | 500 | 1,521,037 B | 5,703,327 B | `8164d622f1aa8731c7d33644a1f7590bb1ff1d9bf75c1889bd44429dedfac591` |
...
shares redundancy across files — the estimate totals 1,700,611 B vs the
actual 1,521,037 B locally built tarball):

| Category | Unpacked B (% of 5,703,327) | Packed est. B (% of est. total) |
|---|---:|---:|
| `docs/` total | 2,303,465 (40.4%) | 618,693 (36.4%) |
| — `docs/research/` | 947,096 (16.6%) | 298,013 (17.5%) |
| — `docs/benchmarks/` | 720,997 (12.6%) | 92,947 (5.5%) |
| — other `docs/` | 635,372 (11.1%) | 227,733 (13.4%) |
| `src/` | 1,125,476 (19.7%) | 383,913 (22.6%) |
| `artifacts/rebaseline-2025/` | 439,339 (7.7%) | 28,029 (1.6%) |
| `scripts/` non-research (27 files) | 327,844 (5.7%) | 107,776 (6.3%) |
| `patterns/` | 314,949 (5.5%) | 127,847 (7.5%) |
| `playground/` + `vercel.json` | 306,156 (5.4%) | 104,586 (6.1%) |
| root files (`CHANGELOG.md` 101,417; `SKILL.md` 53,760; READMEs; config) | 210,181 (3.7%) | 82,828 (4.9%) |
| `tests/quality/` + `tests/fixtures/` | 235,412 (4.1%) | 93,573 (5.5%) |
| `lexicon/` (incl. `provenance/*.json` 56,941) | 95,799 (1.7%) | 14,895 (0.9%) |
| `core/` | 93,206 (1.6%) | 37,966 (2.2%) |
| `assets/` (aside 39,156; social 28,690; brand 3,863; demo README 3,654) | 75,363 (1.3%) | 24,898 (1.5%) |
| `document-types/` | 62,611 (1.1%) | 32,115 (1.9%) |
| `scripts/research/` (6 files) | 62,065 (1.1%) | 20,502 (1.2%) |
| `personas/` | 28,215 (0.5%) | 14,644 (0.9%) |
| `bin/` + `integrations/aside/` | 23,246 (0.4%) | 8,346 (0.5%) |
...
|---|---|---|---|---|---:|---|---|---|---|
| `bin/`, `src/` | CLI implementation | everything | fs/glob via `getRepoRoot()`; fork in `commands/aside.js`; architecture graph | `patina`/`patina-cli`/`patina-score` bins; `docs/CLI.md`; HTTP contract | ~390 KB | plausible/unknown (deep imports) | **KEEP** | n/a (runtime core) | total breakage |
...
| other user-facing `docs/` (50 files: API/CLI/PATTERNS-*/EXAMPLES*/FAQ*/integrations/social/…; 635 KB unpacked) | user documentation, shipped wholesale via `docs/` glob | README links (17–23 per language); `dogfood.mjs` reads `docs/social/*`; `public-showcase.mjs` writes `docs/social/` | fs reads in dogfood; generator writes | installation/support docs the Korean-translation policy pairs; `docs/integrations/*` contracts | ~228 KB | plausible/unknown | **KEEP** | per-file consumer + link audit (large, low value) | broken README/doc links; dogfood gate input disappears |
| development-policy docs subset: `WORKFLOW.md`, `QA.md`, `ARCHITECTURE.md`, `HARNESS.md`, `ROADMAP.md`, `RESEARCH-DOCS-PLATFORM.md` (94,452 B unpacked; `WORKFLOW.md` grew again to 22,643 B, and `QA.md`, `ARCHITECTURE.md`, `ROADMAP.md` also changed with PR #849) | ships because `docs/` ships wholesale; only `docs/internal/**` and `docs/operations/**` are excluded | `HARNESS.md` linked from shipped `tests/quality/README.md`; `ARCHITECTURE.md` cited in source comments; others none found in shipped runtime | doc cross-links only (no fs consumer found) | contributor policy, not a user contract | ~38.9 KB gz est | plausible/unknown | **INSUFFICIENT EVIDENCE** | owner decision + HARNESS link repair in shipped README + registry telemetry; saves ≈1.7% unpacked / ≈2.3% packed est | marginal benefit; breaks shipped README cross-links if removed blindly |
...
saving is ≈1.7% unpacked (94,452 B) / ≈2.3% packed-estimate at best, one of
...
numbers suggest (≈298 + 93 + 28 KB of a 1.52 MB tarball), the whole package
...
## 5. Validation record (audited tree `a1116bf…`; every check re-run at the
final converged tree after the `origin/dev` merge — numbers from `67e0cd8…`
and `3c078b1…` were not reused)

| Command | Result | Exit |
|---|---|---:|
| `npm pack --dry-run --json` (root) | 500 files, 1,521,037 B packed, 5,703,327 B unpacked | 0 |
| `npm pack --dry-run --json` (in `packages/patina-humanizer`) | 3 files, 628 B packed, 972 B unpacked | 0 |
| `node scripts/release-artifacts.mjs --output-dir /tmp/maint07-release-refresh --source-sha a1116bf…` | "Verified 8.7.0 release artifacts" — both tarballs locally built, hashed (root `8164d622…`, alias `63be3af3…`), metadata-verified, and clean-install smoked (no publish; `--publish` not passed) | 0 |
| `npm run benchmark:perf -- --costmetrics` | report-only: npm tarball 1,521,037 B; CLI coldstart 75.05 ms mean over 3 fresh processes; analyzer p50 0.202–2.072 ms across 7 fixtures. Regenerated `docs/benchmarks/perf-latest.{md,json}` in the worktree; snapshot preserved for this audit and the generated files restored to HEAD (generated drift is not part of this PR) | 0 |
