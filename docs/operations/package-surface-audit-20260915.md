...
Read-only audit of what the published npm tarballs actually contain, whether
shipped assets are reachable, and which of them could *safely* be excluded
later. This document records evidence for a future maintainer decision; it
changes no packaging behavior, removes nothing, and creates no new obligation.

- **Audited tree:** `67e0cd85f44500c52f0f708ed83bbfad5db33e63` (`origin/dev`,
  PR #845 merged, main ancestry verified) in worktree `../patina-maint-07`,
  branch `bot/maint-07-package-surface-audit`.
...
## 1. Published surface (measured)

| Package | Files | Packed | Unpacked | Real tarball sha256 |
|---|---:|---:|---:|---|
| `patina-cli@8.7.0` (root) | 500 | 1,519,496 B | 5,698,557 B | `1a99a93ee049c2af83edf6901406e71cba0dde8e0b39306c149e9840d61c73c0` |
...
shares redundancy across files — the estimate totals 1,695,891 B vs the
actual 1,519,496 B tarball):

| Category | Unpacked B (% of 5,698,557) | Packed est. B (% of est. total) |
|---|---:|---:|
| `docs/` total | 2,299,824 (40.4%) | 613,263 (36.2%) |
| — `docs/research/` | 947,096 (16.6%) | 294,724 (17.4%) |
| — `docs/benchmarks/` | 720,997 (12.6%) | 91,986 (5.4%) |
| — other `docs/` | 631,731 (11.1%) | 226,553 (13.4%) |
...
| development-policy docs subset: `WORKFLOW.md`, `QA.md`, `ARCHITECTURE.md`, `HARNESS.md`, `ROADMAP.md`, `RESEARCH-DOCS-PLATFORM.md` (~91 KB unpacked) | ships because `docs/` ships wholesale; only `docs/internal/**` and `docs/operations/**` are excluded | `HARNESS.md` linked from shipped `tests/quality/README.md`; `ARCHITECTURE.md` cited in source comments; others none found in shipped runtime | doc cross-links only (no fs consumer found) | contributor policy, not a user contract | ~30 KB | plausible/unknown | **INSUFFICIENT EVIDENCE** | owner decision + HARNESS link repair in shipped README + registry telemetry; saves ≲2% unpacked | marginal benefit; breaks shipped README cross-links if removed blindly |
...
packed saving is ≈2% at best, one of them (`HARNESS.md`) is cross-linked
...
## 5. Validation record (this audit's tree)

| Command | Result | Exit |
|---|---|---:|
| `npm pack --dry-run --json` (root) | 500 files, 1,519,496 B packed, 5,698,557 B unpacked | 0 |
| `npm pack --dry-run --json` (in `packages/patina-humanizer`) | 3 files, 628 B packed, 972 B unpacked | 0 |
| `node scripts/release-artifacts.mjs --output-dir /tmp/maint07-release-artifacts --source-sha 67e0cd8…` | "Verified 8.7.0 release artifacts" — both tarballs packed, hashed, metadata-verified, and clean-install smoked (no publish; `--publish` not passed) | 0 |
| `npm run benchmark:perf -- --costmetrics` | report-only: npm tarball 1,519,496 B; CLI coldstart 79.098 ms mean over 3 fresh processes; analyzer p50 0.196–2.213 ms across 7 fixtures. Regenerated `docs/benchmarks/perf-latest.{md,json}` in the worktree; snapshot preserved for this audit and the generated files restored to HEAD (generated drift is not part of this PR) | 0 |
| `npm run release:check` | metadata + retired-concepts gates pass | 0 |
| `npm run check:no-private-assets` | "0 forbidden paths across 503 packed + 1387 tracked file(s)" | 0 |
