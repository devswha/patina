...
`patina:monctl:v1:{channel}:{tier}:{suffix}` for synthetic streak, active
alert list, deduplication, and recovery leases; it is
...
service with the monitor channel and `tier=pro`. It must never expose raw
Vercel logs. The safety/entitlement query window is 15 minutes; the
...
missing service, transport failure, non-2xx response, invalid JSON, non-closed
object, or wrongly scoped result is `log_unavailable` and therefore
`monitor_blind`; the monitor returns `503`, never treating it as zero.
...
Configure `PATINA_VERCEL_LOG_QUERY_URL` and `PATINA_VERCEL_LOG_QUERY_TOKEN`
only in the server-side production secret manager. The monitor performs a
read-only GET with bearer authentication to this external service, never
directly to raw Vercel logs. The service derives channel, tier, and requested window from the
...
An absent, malformed, or unavailable service makes the monitor unavailable
(`503`). A successful, correctly scoped result with `monitorDrop >= 3` is
`monitor_blind`, with reason `monitor_drop`.

See [dashboard procedure](../dashboards/pro-launch-v1.md).
