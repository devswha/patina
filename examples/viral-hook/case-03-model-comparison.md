...
| Gemini (Code Assist OAuth) | `gemini -p` (CLI 0.40.0, oauth-personal) | `examples/sample-rewritten-gemini.md` |
...
| **Gemini rewrite** | **30.8** | **73.3** | #1 H, #2 M, #3 H, #5 H |

핵심 관찰: Codex/Claude는 점수를 낮췄지만 **Gemini는 원본(19.6)보다 점수가 더 올라감(30.8)**. 이유는 다음 섹션의 voice 분석에서 다룸. 어쨌든 세 모델 모두 viral-hook 핵심 신호(권위 단언/과장 어휘)는 잔류 — score-only 격리 의도대로.
...
| | Codex (GPT-5.5) | Claude (Sonnet) | Gemini |
|---|---|---|---|
| 본문 위 메타 | 한 줄 ("잔여 AI 티: ...") | 16줄 자기검수 (보존/완화 패턴 명시) | 5개 "AI Tells" 번호 리스트 |
| 메타 형식 | 짤막한 코멘트 | 구조화된 섹션 (선언적) | 패턴별 분석 (해설형) |

→ Claude는 self-audit을 가장 충실히. Gemini는 "탐지된 패턴" 목록을 본문 앞에 두는 분석 톤. Codex는 형식적 한 줄.

### 핵심 단어 선택

| 원문 어휘 | Codex | Claude | Gemini |
|---|---|---|---|
| "역사상 이런 속도는 없었다" | "거의 못 봤다" | (삭제 후 사실 진술) | "전례 없는 속도입니다" |
| "미친 듯이 달려든" | "한꺼번에 몰렸다" | (삭제 + 인과 재진술) | "왜 이렇게 열광할까요?" (질문화) |
| "...이유가 뭘까" | (제거) | (제거) | **유지** ("왜 이렇게 열광할까요?"로 변형) |
| "99%는 ... 상위 1%는" | "대부분은 ... 움직이는 사람은" | "대부분은 ... 일부는" | "대다수는 ... 감이 빠른 사람들은" |
| "가장 싸게 올라탈 타이밍" | "가장 싸게 배울 수 있는 타이밍" | "제일 싸게 타는 타이밍" | "가장 좋은 기회입니다" |
| "도구는 죄가 없다 / 게으름이 문제" | (큰 변경 없음) | **삭제** | "결국 실행력 차이일 뿐이죠" (순화 보존) |

### 종결 어미·register

- **Codex**: `~다` 일관적 평서. 중성·뉴스 톤.
- **Claude**: `~다` 본문 + 마지막만 캡션 클로저 — register layering 의식.
- **Gemini**: **`~습니다`/`~죠`/`~까요?` 격식체** 일관 — 한국어 textbook formal 톤. 다른 두 모델과 가장 큰 차이.

### 문장·단락 구조

- **Codex**: 1줄 단문 일부 묶음. 보수적.
- **Claude**: 적극 통합 + burstiness 의식 (self-audit에 명시).
- **Gemini**: 단락은 비교적 보존, 대신 **호흡 최적화 단문 → 격식 만연체**로 변환. 결과적으로 마케팅 임팩트는 약해지나 격식체 특유의 AI 패턴(긴 명사구, 정중한 종결)이 들어옴.
...
- `examples/sample-rewritten-gemini.md` — Gemini/Code Assist rewrite
