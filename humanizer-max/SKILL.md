...
`.humanizer.yaml` (사용자 설정 파일)이 프로젝트 루트에 없으면 최초 실행으로 간주하고 `AskUserQuestion`으로 인터뷰를 진행한다:
...
인터뷰 결과를 `.humanizer.yaml`에 저장한다:

```yaml
# Generated by humanizer-max setup interview
max-models:
  - claude
  - gemini
dispatch: omc
...
이후 실행에서는 `.humanizer.yaml`이 존재하면 인터뷰를 건너뛰고 바로 설정을 로드한다. `.humanizer.default.yaml`보다 `.humanizer.yaml`의 값이 우선한다.
...
3. 프로필의 `voice-overrides`와 목소리 지침에 따라 필자의 개성을 불어넣어라
4. 핵심 메시지와 의미를 반드시 보존하라

## Phase 3: 자기검수 (Self-Audit)

Phase 2 출력 전체를 읽고 스스로 물어라: "아래 글에서 AI가 쓴 것처럼 보이는 부분은?"

1. 아직 AI처럼 느껴지는 표현이 있으면 마지막으로 수정하라
2. 수정 후 다시 검토하여 더 이상 AI 패턴이 없으면 완료한다
