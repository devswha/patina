...
export const EXPERIENCE_COPY = {
  en: {
    ...
    pricingNote: 'The key from your purchase email is the only thing you need — paste it into Pro mode and your first rewrite checks it. We never save it. The monthly count and the monthly character total are separate limits, so either one can stop you first.',
  ...
  ko: {
    ...
    pricingNote: '구매 메일로 받은 키 하나면 됩니다. Pro에 붙여넣으면 처음 다듬을 때 확인해요. 키는 저장하지 않습니다. 한 달 횟수와 한 달 글자 수는 따로 세기 때문에, 둘 중 하나가 먼저 찰 수 있어요.',
  ...
  zh: {
    ...
    pricingNote: '购买邮件里的那把密钥就够了。粘进 Pro，第一次改写时会核对。我们不保存它。每月次数和每月字数是分开算的，哪一项先用完都会停下来。',
  ...
  ja: {
    ...
    pricingNote: '購入メールに届いたキーだけで使えます。Pro に貼り付けると、最初の書き換えで確認します。キーは保存しません。1 か月の回数と文字数は別々に数えるので、どちらかが先に上限に達することがあります。',
