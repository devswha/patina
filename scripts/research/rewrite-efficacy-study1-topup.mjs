// Top-up pass for Study 1 (see rewrite-efficacy-topup.mjs for the pilot
// version and the rationale: transient backend failures leave NON-RANDOM holes,
// and Krippendorff's alpha loses a whole agreement unit per missing rating).
//
// Differences from the pilot top-up:
//   - 3-judge panel (kimi / gpt / grok), matching the Study 1 pre-registration
//   - shard glob accepts both sharded (s1-rows-A1.jsonl) and unsharded
//     (s1-rows.jsonl) files — the pilot top-up silently skipped unsharded runs
//   - repairs count per judge so the analyzer's transparency block can report
//     exactly how much of the matrix was topped up
//
// Run with nothing else competing for the local CLIs.
// Usage: node scripts/research/rewrite-efficacy-study1-topup.mjs [--dry-run]
...
const LOG = join(DIR, 's1-topup.log');
...
const JUDGES = [
  ...
  { id: 'judge-gpt', cmd: 'codex', args: CODEX_ARGS },
  { id: 'judge-grok', cmd: 'node', args: [join('scripts', 'research', 'xai-cli.mjs')] },
];
...
async function main() {
  const shards = readdirSync(DIR).filter((f) => /^s1-rows(-.*)?\.jsonl$/.test(f));
  const textIndex = new Map();
  for (const f of readdirSync(DIR).filter((x) => /^s1-texts(-.*)?\.private\.jsonl$/.test(x))) {
    for (const t of readJsonl(join(DIR, f))) textIndex.set(`${t.arm}:${t.original_sha}`, t);
  }

  const repairedByJudge = Object.fromEntries(JUDGES.map((j) => [j.id, 0]));
      ...
      if (row.rewrite_error && !row.rewrite_sha) {
        ...
        if (rw.text) {
          ...
          for (const j of JUDGES) {
            ...
            if (parsed) repairedByJudge[j.id] += 1;
          }
          appendFileSync(join(DIR, 's1-texts-TOPUP.private.jsonl'), JSON.stringify(stored) + '\n');
          repairedRewrites += 1;
          changed = true;
  ...
  const perJudge = JUDGES.map((j) => `${j.id}:${repairedByJudge[j.id]}`).join(' ');
  log(`top-up complete: judges repaired [${perJudge}], ${repairedRewrites} rewrites repaired; ${stillBroken} still broken${DRY ? ' (DRY RUN)' : ''}`);
}
