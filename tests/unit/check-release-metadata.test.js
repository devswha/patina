...
for (const path of README_FILES) {
  ...
  test(`collector rejects ${path} catalog total drift`, () => {
    withFixture((root) => {
      updateReadme(root, path, (readme) => readme.replace('172', '168'));
      assert.deepEqual(collectReleaseMetadataErrors({ repoRoot: root }).errors, [`${path} catalog must match the canonical 172-pattern breakdown`]);
    });
  });

  test(`collector rejects ${path} catalog breakdown drift`, () => {
    withFixture((root) => {
      updateReadme(root, path, (readme) => readme.replace('34', '33'));
      assert.deepEqual(collectReleaseMetadataErrors({ repoRoot: root }).errors, [`${path} catalog must match the canonical 172-pattern breakdown`]);
    });
  });
}
