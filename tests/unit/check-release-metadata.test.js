...
for (const path of README_FILES) {
  ...
  test(`collector rejects ${path} catalog total drift`, () => {
    withFixture((root) => {
      updateReadme(root, path, (readme) => readme.replace(String(CATALOG_TOTAL), '168'));
      assert.deepEqual(collectReleaseMetadataErrors({ repoRoot: root }).errors, [`${path} catalog must match the canonical ${CATALOG_TOTAL}-pattern breakdown`]);
    });
  });
