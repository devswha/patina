...
test('validator rejects every held env-name, blocker, decision, deferred action, and freeze hash mutation', () => {
  ...
  const suppliedVercelHash = validSources(); delete suppliedVercelHash.checkHashes; suppliedVercelHash.hashFile = (file) => file === 'vercel.json' ? '0'.repeat(64) : original.frozenSemantics.sourceHashes[file]; assert.match(validatePreflightHold(original, suppliedVercelHash).join('\n'), /frozen SHA-256 mismatch: vercel\.json/);
