...
test('TIER_LIMITS.pro documents the pro caps without regressing free/byok', () => {
  assert.deepEqual(TIER_LIMITS.pro, { maxChars: 20000, reqPerDay: 200, reqPerMonth: 100, maxConcurrent: 3, charsPerMonth: 50000 });
...
test('resolveTierLimits returns the defaults when no env overrides are present', () => {
  ...
  assert.deepEqual(limits.pro, { maxChars: 20000, reqPerDay: 200, reqPerMonth: 100, maxConcurrent: 3, charsPerMonth: 50000 });
...
test('resolveTierLimits applies positive-integer pro env overrides', () => {
  const limits = resolveTierLimits({
    ...
    PATINA_PRO_REQ_PER_MONTH: '500',
  });
  assert.deepEqual(limits.pro, { maxChars: 50000, reqPerDay: 1000, reqPerMonth: 500, maxConcurrent: 8, charsPerMonth: 2000000 });
...
test('resolveTierLimits falls back to defaults for invalid pro overrides', () => {
  for (const bad of ['0', '-5', 'abc', '3.5', '', ' ', undefined]) {
    ...
    assert.deepEqual(
      limits.pro,
      { maxChars: 20000, reqPerDay: 200, reqPerMonth: 100, maxConcurrent: 3, charsPerMonth: 50000 },
